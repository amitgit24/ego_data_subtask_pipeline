...
Adapted from Kinematics_pipeline's postprocess.py. The cross-check is
necessarily different: there is no independent kinematic signal to check a
verb against here, so instead of `kinematic_mismatch` this pipeline flags
`short_segment` — a segment whose duration is a small fraction of its
episode's median, computed purely in code from the segments themselves, as
a soft "this one might be a spurious transition" signal.
"""
...
def enforce_min_length(segments, min_len):
    """Absorb every sub-`min_len` segment into its better neighbor until all
    survivors meet the floor (or only one segment remains).

    Level 1 here emits raw VLM transitions with no minimum-gap rule (unlike
    Kinematics_pipeline, which enforces min length while proposing
    boundaries), so two transitions a few frames apart can produce a segment
    shorter than min_segment_frames. Such a segment is almost always a
    spurious/over-eager transition; merge it into the adjacent segment with
    the higher-confidence label (tie -> the longer one), keeping that label
    and the union of bounds so coverage stays total and contiguous. The
    merge is by TIMING only and ignores action/object/hand, which is what
    makes it able to remove a spurious boundary that merge_segments (which
    requires matching labels) cannot."""
    ...
    while len(segs) > 1:
        ...
        if lengths[shortest] >= min_len:
            break
        # choose the neighbor to merge into
