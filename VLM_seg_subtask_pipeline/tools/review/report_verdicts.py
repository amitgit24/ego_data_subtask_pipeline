"""Score exported human verdicts against the Qwen-direct pipeline annotations.

    python report_verdicts.py path/to/qwen_direct_review_verdicts.json

Sibling of ../../../tools/review/report_verdicts.py (the Kinematics scorer):
identical metrics so the two pipelines' human-review results are directly
comparable — accuracy per axis (boundary / label / sentence / episode
coherence), label accuracy per action verb, and flag calibration: do this
pipeline's review-queue flags (low_confidence / short_segment) actually
predict human-found errors?
"""
...
def main():
    ...
    flag_cal = {True: [], False: []}   # flagged? -> label verdicts
    episode_ok = []
    notes = []
    ...
    print("======= QWEN-DIRECT HUMAN VERDICT REPORT =======")
