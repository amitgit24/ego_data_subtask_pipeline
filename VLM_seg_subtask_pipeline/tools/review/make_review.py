"""Human review UI builder for the Qwen-direct pipeline.

Sibling of ../../../tools/review/make_review.py (the Kinematics reviewer).
Same self-contained review page and verdict axes so the two pipelines are
judged by an IDENTICAL human protocol — the whole point of running them in
parallel is a fair head-to-head, and that requires the reviewer to answer the
same questions about both. What differs is only the source of annotations
(this pipeline's outputs/annotations) and the flag vocabulary shown
(low_confidence / short_segment instead of the kinematic ones), which the UI
renders generically from each subtask's `flags` list.

Selects annotated episodes (half containing review-flagged subtasks, half
clean — so the flag-calibration question gets answered), transcodes each
episode video to a small browser-playable H.264 MP4, and emits ONE
self-contained page: segment timeline synced to the video, per-segment play
(with +-0.5 s pre/post-roll for boundary judgment), verdict buttons per
segment (boundary / label / sentence) plus an episode-level coherence verdict.
Verdicts persist in localStorage under a pipeline-specific key (so they do not
collide with the Kinematics reviewer in the same browser); the Export button
...
<title>EgoDex Qwen-direct annotation review</title>
<style>
...
<div id="main">
 <div id="topbar">
  ...
  <span class="badge">pipeline: Qwen-direct</span>
...
<script>
const DATA = %%DATA%%;
const LS = 'egodex_qwen_direct_review_v1';
...
$('export').onclick = ()=>{
  const blob = new Blob([JSON.stringify({version:1, pipeline:'qwen_direct', exported_at:new Date().toISOString(), verdicts:store}, null, 2)], {type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob); a.download = 'qwen_direct_review_verdicts.json'; a.click();
};
renderMain();
</script></body></html>
"""
