"""Level 3 — output schema, identical in shape to the sibling pipelines so all
three are directly comparable. All timing is computed in code from frame
indices and the probed FPS; the model validator re-derives and enforces it.

Only the boundary_source enum differs: here segment boundaries come from the
single-pass VLM generator ("vlm_subtask") plus code-inserted idle gaps. Flags
match VLM_seg_subtask_pipeline (low_confidence / short_segment) — there is no
independent kinematic signal to cross-check against.
"""
...
BoundarySource = Literal["vlm_subtask", "idle_gap", "episode_start",
                         "episode_end"]
