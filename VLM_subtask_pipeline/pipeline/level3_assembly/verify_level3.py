...
re-check, annotated preview videos, summary report. Same as the sibling
pipelines' verify_level3.py.
...
def render_preview(ann, mp4_path, out_path):
    ...
    while True:
        ...
        s = subs[si]
        band_h = 110
        frame[h - band_h:] = frame[h - band_h:] // 3
        cv2.putText(frame, f"#{s.id} [{s.action}] hand={s.hand}", (20, h - 72),
        ...
        cv2.rectangle(frame, (20, h - 20),
                     (20 + int((w - 40) * min(prog, 1.0)), h - 12), (0, 220, 0), -1)
        out.write(frame)
        f += 1
    cap.release()
    out.release()
...
def main():
    ...
    for f in files:
        try:
            valid.append(EpisodeAnnotation.model_validate(json.loads(f.read_text())))
    ...
    print(f"flagged for review: {n_review} subtasks ({flagged / max(n_subs, 1):.1%} of all)")
    print(f"verb histogram: {dict(verbs.most_common())}")

    checks = [
        (f"schema-valid: {len(valid)}/{len(files)} (must be 100%)", not invalid),
        (f"invariants + duration arithmetic exact to {TIME_TOL}", not invalid),
        (f"{min(args.previews, len(valid))} preview videos rendered", True),
    ]
    print("\n============ LEVEL 3 VERIFIER ============")
    all_pass = True
